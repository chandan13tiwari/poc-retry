...
public class RetryServiceImpl implements RetryService{
    ...
    public void pollDelayForRetry(long delay, ChronoUnit timeUnit) {
        Awaitility.await()
                .timeout(delay + 1, TimeUnit.of(timeUnit))
                .pollDelay(delay, TimeUnit.of(timeUnit))
    ...
    public void pollIntervalForRetry(long interval, ChronoUnit timeUnit) {
        Awaitility.await()
                .timeout(interval + 1, TimeUnit.of(timeUnit))
                .pollInterval(interval, TimeUnit.of(timeUnit))
                .untilAsserted(() -> Assert.assertTrue(true));
    }
}
